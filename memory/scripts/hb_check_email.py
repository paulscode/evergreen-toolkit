...
# Load credentials
CRED_FILE = os.path.join(os.path.expanduser("~"), ".openclaw/workspace/.gmail_imap.json")
...
def store_email_memory(user_id, sender, subject, body, date):
    ...
    try:
        ...
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "background_store.py")
        subprocess.Popen([
            "python3", script_path,
            f"[Email] {subject}",
            email_text,
...
def get_user_context(user_id):
    ...
    try:
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "get_user_context.py")
    ...
    try:
        ...
        for eid in email_ids[-10:]:
            ...
            if user_id:
                ...
                # Output for the agent to respond (with context hint)
