...
Usage: python3 save_mem.py [--user-id yourname] [--reset]
"""
...
# Add script directory to path for memory_validation
SCRIPT_DIR = Path(__file__).parent
sys.path.insert(0, str(SCRIPT_DIR))

try:
    from validate_memory import validate_content, wrap_external_content
...
if not USER_ID:
    sys.exit("Error: USER_ID or DEFAULT_USER_ID must be set in .memory_env")
                    ...
                    if entry.get('type') == 'message' and 'message' in entry:
                        ...
                        if content and role in ('user', 'assistant'):
                            ...
                            if VALIDATION_AVAILABLE:
                                is_safe, validation_warnings = validate_content(content)
                                if not is_safe:
                                    # Log warning but still save (don't block legitimate content)
                                    print(f"⚠️ Injection pattern detected in turn {turn_counter + 1}: {validation_warnings}", file=sys.stderr)
