...
MEMORY_DIR = os.path.join(os.getenv("OPENCLAW_WORKSPACE", os.path.join(os.path.expanduser("~"), ".openclaw", "workspace")), "memory")

# DEFAULT USER - Mem0-style: memories belong to user
DEFAULT_USER_ID = os.getenv("DEFAULT_USER_ID", "yourname")
...
def parse_daily_log(date_str: str, user_id: str = None) -> List[Dict[str, str]]:
    """Parse the daily memory file into conversation turns"""
    # Check per-user directory first, then global
    if user_id:
        user_log = os.path.join(MEMORY_DIR, user_id, f"{date_str}.md")
    ...
    for section in sections:
        ...
        ai_match = re.search(rf'(?:{re.escape(agent_name)}|Assistant|AI)[:\s]+(.+?)(?=\n##|\Z)', body, re.DOTALL | re.IGNORECASE)
        if ai_match:
...
def store_conversation_turn(
    ...
    points.append({
        ...
        "payload": {
            "user_id": user_id,
            "text": f"[{os.getenv('AGENT_NAME', 'Assistant')}]: {ai_response}",
            "date": date_str,
...
def main():
    ...
    conversations = parse_daily_log(date_str, user_id=user_id)
