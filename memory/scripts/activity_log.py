#!/usr/bin/env python3
"""
Shared Activity Log for agent and evergreen
...
# Embedding function (simple keyword-based for dedup matching — not semantic search)
# This uses hash-based approximate matching which is sufficient for duplicate detection.
# For true semantic search, use search_memories.py or hybrid_search.py instead.
def simple_embed(text: str) -> list[float]:
    """Simple hash-based embedding for duplicate detection.
    Sufficient for dedup but not for semantic search — use Ollama embeddings for that."""
    # In production, use snowflake-arctic-embed2 via Ollama API
...
def log_activity(
    ...
    Args:
        agent: Your agent's name or "evergreen"
...
def get_recent_activities(
    ...
    Args:
        agent: Filter by agent name or None for all
...
def main():
    parser = argparse.ArgumentParser(description="Shared Activity Log for agents")
    subparsers = parser.add_subparsers(dest="command", help="Command to run")
    
    # Log command
    log_parser = subparsers.add_parser("log", help="Log an activity")
    log_parser.add_argument("--agent", required=True, help="Which agent performed the action (e.g., your agent name or 'evergreen')")
    ...
    if args.command == "log":
        if args.check_duplicate:
            ...
            if check_for_duplicates(args.action, keywords):
                if sys.stdin.isatty():
                    ...
                    print("Duplicate detected in non-interactive mode. Use --no-dedup to skip check.", file=sys.stderr)
                    sys.exit(1)
        ...
        for a in activities:
            agent_icon = "🤖" if a["agent"] == "evergreen" else "🎙️"
            status_icon = {
                "completed": "✓",
                "in_progress": "◐",
                "blocked": "✗",
