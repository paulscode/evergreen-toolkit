...
AGENT_NAME = os.getenv("AGENT_NAME", "Assistant")  # Set via AGENT_NAME env var in .memory_env
...
def check_other_agent(action_type: str, keywords: str, hours: int = 6) -> bool:
    """
    Check if evergreen (or Eve) already did this recently
    
    Example:
        if check_other_agent("cron_created", "openclaw repo monitoring"):
            print("evergreen already set this up!")
            return
    """
    other_agent = "evergreen" if AGENT_NAME != "evergreen" else "Assistant"
...
def show_recent_collaboration(hours: int = 24):
    ...
    for a in activities:
        agent = a['agent']
        icon = "🤖" if agent == "evergreen" else "🎙️"
