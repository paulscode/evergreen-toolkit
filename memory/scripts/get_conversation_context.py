...
Cross-conversation search across all of the user's memories.

Usage:
    # Search user's memories across all conversations
    python3 scripts/get_conversation_context.py --user-id "yourname" "what was the decision about Qdrant?"
    
    # Get specific conversation
    python3 scripts/get_conversation_context.py --user-id "yourname" --conversation-id <id>
    
    # Get all conversations for user
    python3 scripts/get_conversation_context.py --user-id "yourname" --limit 50
...
def get_embedding(text: str) -> Optional[List[float]]:
    ...
    data = json.dumps({
        "model": os.getenv("EMBEDDING_MODEL", "snowflake-arctic-embed2"),
        "input": text[:8192]
    }).encode()
...
def main():
    ...
    parser.add_argument("--user-id", required=True,
                        help="REQUIRED: User ID (e.g., 'yourname')")
