...
Usage: store_memory.py "Memory text" [--tags tag1,tag2] [--importance medium] 
                         ...
                         [--verified] [--expires 2009-01-03] [--related id1,id2]
...
def store_single_memory(
    ...
    # Only check for duplicates when a specific date was provided (e.g., daily
    # summary imports).  Real-time notes (no explicit date) should always store.
    explicit_date = date is not None
    if date is None:
        date = datetime.now().strftime("%Y-%m-%d")
    
    # Check for existing entry on same date (only for explicit-date uploads)
    existing_id = check_existing(date=date) if explicit_date else None
...
def main():
    ...
    parser.add_argument("--user-id", help="User ID for memory isolation (stored as payload field)")
