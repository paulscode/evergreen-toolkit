...
Usage: python3 search_mem.py "your search query" [--limit 10] [--user-id yourname]

Searches:
1. Redis (mem:{user_id}) - exact text match in recent buffer
2. Qdrant (QDRANT_COLLECTION, default: agent-memories) - semantic similarity search
"""
...
USER_ID = os.getenv("USER_ID", os.getenv("DEFAULT_USER_ID", "yourname"))
...
def search_qdrant(query, user_id, limit=10):
    ...
    collection = os.getenv("QDRANT_COLLECTION", "agent-memories")
    req = urllib.request.Request(
